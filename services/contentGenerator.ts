...
import { getChapterData, getCustomSyllabus, incrementApiUsage, getApiUsage } from "../firebase";
...
const executeBulkParallel = async <T>(
    ...
    console.log(`🚀 Starting Bulk Engine (AI): ${tasks.length} tasks (Parallelism: ${concurrency})`);
...
export const translateToHindi = async (content: string, isJson: boolean = false, usageType: 'PILOT' | 'STUDENT' = 'STUDENT'): Promise<string> => {
    ...
    return await executeWithRotation(async () => {
        return await aiOrchestrator.callAI([{ role: "user", content: prompt }], 'TRANSLATION');
    }, usageType);
};
...
export const fetchChapters = async (
  ...
  try {
    const data = await executeWithRotation(async () => {
        const content = await aiOrchestrator.callAI([
...
export const fetchLessonContent = async (
  ...
  // modelName logic removed as AI Orchestrator handles it
  let promptNotes = "";
  ...
  if (dualGeneration && (type === 'NOTES_PREMIUM' || type === 'NOTES_SIMPLE')) {
       ...
       const rawText = await executeWithRotation(async () => {
          return await aiOrchestrator.callAI([{ role: "user", content: prompt }], 'NOTES');
       }, usageType);
