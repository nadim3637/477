...
export interface SystemSettings {
  ...
  voiceGuides?: Record<string, string>; // NEW: Voice scripts for buttons
...
export type AIProvider = 'openai' | 'gemini' | 'groq' | 'openrouter' | 'anthropic' | 'mistral' | 'cohere' | 'huggingface' | 'local' | 'deepseek' | 'together' | 'fireworks' | 'perplexity';

export interface AIModel {
  id: string; // "gpt-4", "llama-3-8b"
  ...
  maxTokens?: number;
  isFallback?: boolean; // If true, only used when others fail
  contextWindow?: number;
  apiKey?: string; // Optional override
}

export interface AITaskConfig {
  taskType: 'NOTES' | 'MCQ' | 'CHAT' | 'ANALYSIS' | 'TRANSLATION' | 'VOICE';
  preferredModelId?: string;
  fallbackModelIds?: string[]; // Order of fallback
  temperature?: number;
}
